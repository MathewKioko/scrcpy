...
#define SC_OPTION_LIST_APPS 0x10
    ...
    // MathewKioko Enhanced Features
    struct sc_advanced_recording_options advanced_recording;
    struct sc_enhanced_ui_options enhanced_ui;
    bool enable_performance_monitoring;
    bool enable_network_optimization;
    bool enable_auto_update_check;
    bool enable_crash_reporting;
    bool enable_advanced_logging;
    const char *custom_config_path;
    bool enable_multi_device_support;
    bool enable_device_grouping;
    bool enable_quick_actions;
    bool enable_custom_shortcuts;
    const char *shortcut_config_file;
};
...
enum sc_advanced_recording_mode {
    SC_ADVANCED_RECORDING_NORMAL,
    SC_ADVANCED_RECORDING_SPLIT,
    SC_ADVANCED_RECORDING_SCHEDULED,
    SC_ADVANCED_RECORDING_CONTINUOUS,
};

struct sc_advanced_recording_options {
    enum sc_advanced_recording_mode mode;
    uint32_t split_duration; // in seconds
    bool schedule_enabled;
    uint8_t schedule_hours[24]; // bitmask for hours to record
    uint32_t max_storage_gb; // maximum storage to use
    bool compress_on_save;
};

struct sc_enhanced_ui_options {
    enum sc_theme theme;
